...
class CustomEnv(gym.Env):
    ...
    def __init__(self, grid_size: int):
        ...
        self.observation_space = spaces.Box(low=0, high=1,
                                            shape=(2, resolution, resolution), dtype=np.float32)
    ...
    def _make_observation(self):
        ...
        observation = np.zeros((2, resolution, resolution), dtype=np.float32)
        observation[0, self.position_self_x * self.scale:(self.position_self_x + 1) * self.scale,
                    self.position_self_y * self.scale:(self.position_self_y + 1) * self.scale] = 1
        observation[1, self.position_goal_x * self.scale:(self.position_goal_x + 1) * self.scale,
                    self.position_goal_y * self.scale:(self.position_goal_y + 1) * self.scale] = 1
        return observation
